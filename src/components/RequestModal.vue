<template>
  <Teleport to="body">
    <div class="fixed inset-0 bg-[rgba(0,0,0,0.6)] backdrop-blur-sm flex items-center justify-center z-[1000] p-12"
      @click.self="handleClose">
      <div
        class="bg-white border-2 border-black/15 rounded-2xl shadow-xl min-w-[800px] max-w-[1200px] max-h-[90vh] relative flex flex-col overflow-hidden outline-none">
        <!-- Header -->
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
          ...
          <button
            ...
            @click="handleClose" aria-label="Close dialog">
            ×
          </button>
        </div>
        ...
        <div class="px-6 py-6 overflow-y-auto flex-1 min-h-0">
          ...
          <div class="mb-5">
            ...
            <select v-model="selectedServiceItemIndex"
          ...
          <div class="mb-5">
            <label class="block mb-2 text-sm font-medium text-gray-700">Comments:</label>
            <input type="text" v-model="comment" placeholder="Enter comments"
              class="w-full h-9 px-4 py-2 border border-gray-300 rounded-lg bg-white text-sm text-gray-900 font-medium shadow-sm transition-all hover:border-gray-400 hover:shadow focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
          </div>

          <!-- Tabulator Grid -->
          <div ref="modalGridContainer"
            class="flex-1 min-h-[400px] my-5 border border-gray-300 rounded-lg overflow-hidden"></div>
...
const serviceItemCode = computed(() => {
  const si = serviceItems.value.find(si => si.siIndex === selectedServiceItemIndex.value)
  return si ? si.code : ''
})
  ...
  try {
    ...
    const requestData = {
      data: requestRows.value.map(row => ({
        ...
        service_item_code: serviceItemCode.value,
        job_sample_code: row.sampleId,
        traveller_position: row.position,
        crm_code: row.controlType.toLowerCase() === 'crm' ? row.sampleName : '',
      })),
      comments: comment.value,
    }

    const endpoint = "/api/qc-results/create-qc-traveller"
